...
{
    public partial class FormLevelCycle : Form
    {
        public FormLevelCycle()
        {
            InitializeComponent();
        ...
        public double[,] coor =
        {
            ...
            { winW + (  -108.253 + offsetX ) * directX , winH + ( -100 + offsetY ) * directY },
            { winW + ( -62.5 + offsetX ) * directX , winH + ( -173.205 + offsetY ) * directY },
            { winW + ( 0 + offsetX ) * directX ,winH +  ( -200 + offsetY ) * directY },
            { winW + ( 62.5 + offsetX ) * directX , winH + ( -173.205 + offsetY ) * directY },
            { winW + (  108.253 + offsetX ) * directX , winH + ( -100 + offsetY ) * directY },
            { winW + (  125 + offsetX ) * directX , winH + ( 0 + offsetY ) * directY },
            { winW + (  108.253 + offsetX ) * directX , winH + ( 100 + offsetY ) * directY },
        ...
        private void FormLevelCycle_Load(object sender, EventArgs e)
        {
        ...
        private void paint(object sender, PaintEventArgs e) //complete graph畫線
        {
            ...
            using (Graphics g = e.Graphics)
            {
                ...
                for (int i = 0; i < pointnum; i++)
                {
                    ...
                    {
                        g.DrawLine(p, new Point((int)(coor[i, 0] + pSizeW / 2), (int)(coor[i, 1] + pSizeH / 2)), new Point((int)(coor[(i + 1 >= pointnum ? 0 : i + 1), 0] + pSizeW / 2), (int)(coor[(i + 1 >= pointnum ? 0 : i + 1), 1] + pSizeH / 2)));
                    }
                }
            }
        }
        ...
        private void picturebox_enter(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < pointnum; i++)
            {
                if (i == ((name + 1) % pointnum) || i == ((name + pointnum - 1) % pointnum))
                {
                    if ((int)SS[i].Tag <= 3)
                    {
